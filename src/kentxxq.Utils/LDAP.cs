...
namespace kentxxq.Utils
{
    public static class LDAP
    {
        ...
        /// <param name="verifySSL">是否验证ssl证书 默认不验证，因为很多都是自签发的。如果需要验证，请在客户端机器导入证书。</param>
        /// <returns>状态正常为true，否则false</returns>
        public static bool VerifyLdapConnection(string server, int port, string userDN, string password, bool enableSSL = false, bool verifySSL = false)
        {
            ...
            // 如果启用ssl，这一行必须要有
            if (enableSSL) ldapConn.SecureSocketLayer = true;
